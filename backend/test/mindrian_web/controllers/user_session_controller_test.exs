defmodule MindrianWeb.UserSessionControllerTest do
  ...
  describe "POST /users/log-in - email and password" do
    test "logs the user in", %{conn: conn, user: user} do
      ...
      # Now do a logged in request - should serve SPA
      conn = get(conn, ~p"/")
      response = html_response(conn, 200)
      # SPA is served; user data is loaded via API
      assert response =~ "mindrian-frontend"
    end
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_mindrian_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "POST /users/log-in - magic link" do
    test "sends magic link email when user exists", %{conn: conn, user: user} do
      ...
      assert Mindrian.Repo.get_by!(Accounts.UserToken, user_id: user.id).context == "login"
    end
